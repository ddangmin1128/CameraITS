...
import cjh_get_IPADDR as _get_ip
import mobly.controllers.android_device_lib.snippet_client_v2 as _scv2

def _patched_create_socket_connection(self):
  _host = _get_ip.get_tailscale_ip()
  try:
    self._conn = _socket.create_connection((_host, self.host_port),
                                           _scv2._SOCKET_CONNECTION_TIMEOUT)
  except Exception:
    self._conn = _socket.create_connection((_host, self.host_port),
                                           _scv2._SOCKET_CONNECTION_TIMEOUT)
  self._conn.settimeout(_scv2._SOCKET_READ_TIMEOUT)
  self._client = self._conn.makefile(mode='brw')

_scv2.SnippetClientV2.create_socket_connection = _patched_create_socket_connection
#/cjh
...
class ItsBaseTest(base_test.BaseTestClass):
  ...
  def setup_tablet(self):
    ...
    #self.set_tablet_landscape_orientation() #MoonITS
