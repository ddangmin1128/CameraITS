...
import cjh_get_IPADDR as getCurrentIp
...
class ItsSession(object):
  ...
  def __init_socket_port(self):
    ...
    socket_lock.close()
    # self.IPADDR = "100.68.38.98" #cjh
    #cjh
    self.IPADDR = getCurrentIp.get_tailscale_ip()
    # current_ip = f"Current IP : {self.IPADDR}" 
  ...
  def reset_socket_and_camera(self):
    ...
    self.IPADDR = getCurrentIp.get_tailscale_ip()   # cjh
...
def copy_scenes_to_tablet(scene, tablet_id):
  ...
  for file_name in scene_dir:
    if file_name.endswith('.png') or file_name.endswith('.mp4'):
      ...
      subprocess.Popen(cmd.split()) #MoonITS
