...
def _is_live_log_enabled():
  """Returns True when run_all_tests should write exhibition live logs."""
  return os.environ.get('ITS_ENABLE_LIVE_LOG', '1') != '0'


def _get_live_log_paths():
  """Returns the fixed DEBUG/INFO live log paths."""
  live_log_dir = os.environ.get(
      'ITS_LIVE_LOG_DIR',
      os.path.join(tempfile.gettempdir(), LIVE_LOG_DIRNAME),
  )
  live_log_prefix = os.environ.get('ITS_LIVE_LOG_PREFIX', LIVE_LOG_PREFIX)
...
def _write_live_log_line(debug_file, info_file, line):
  """Writes one line into the exhibition live log files."""
...
def main():
  ...
  if live_logging_enabled:
    ...
    live_info_file = open(live_info_path, 'a', encoding='utf-8')
    atexit.register(live_debug_file.close)
    atexit.register(live_info_file.close)
  try:
  ...
  if live_logging_enabled:
    logging.info('Saving live exhibition logs to: %s and %s',
                 live_debug_path, live_info_path)
  ...
  for camera_id in camera_id_combos:
    ...
    for s in per_camera_scenes:
      ...
      os.makedirs(mobly_scene_output_logs_path, exist_ok=True)
      num_pass = 0
      num_skip = 0
      ...
      for test in scene_test_list:
        ...
        stdout_path = os.path.join(
            mobly_scene_output_logs_path, f'{test_name}{LIVE_STDOUT_SUFFIX}')
        return_string = ''
        for num_try in range(NUM_TRIES):
          ...
          if live_logging_enabled:
            _write_live_log_line(
                live_debug_file,
                live_info_file,
                (f'LIVE_LOG_START scene={s} camera=cam_id_{camera_id_str} '
                 f'test={test_name} output={mobly_scene_output_logs_path}'),
            )
          ...
          with output.stdout, open(
          ...
          ) as file, open(stdout_path, 'ab') as stdout_file:
            for line in iter(output.stdout.readline, b''):
              ...
              stdout_file.flush()
              if live_logging_enabled and out:
                _write_live_log_line(
                    live_debug_file,
                    live_info_file,
                    f'[{s}/{test_name}] {out}',
